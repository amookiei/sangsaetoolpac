...
export function Dashboard() {
  ...
  return (
    <div className="dash">
      <div className="hero">
        <h1>오늘은 어떤 상세페이지를 만들까요?</h1>
        <p>기획안을 넣으면 구조 설계부터 후킹멘트, 디자인, SVG/PNG 추출까지 7단계로 완성됩니다.</p>
      </div>

      <div className="card">
        <div className="section-title" style={{ marginTop: 0 }}>새 프로젝트 만들기</div>
        <label className="label">제품/서비스 이름</label>
        <input
          className="input"
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {CATEGORIES.map((c) => (
            ...
            >
              {CATEGORY_LABEL[c]}
              <span className="badge" style={{ marginLeft: 2 }}>학습 {refCount(platform, c)}</span>
            </button>
          ))}
        </div>
      ...
      {projects.length === 0 && <div className="hint">아직 프로젝트가 없어요. 위에서 첫 프로젝트를 만들어 보세요!</div>}
          ...
          return (
            <div key={p.id} className="card proj-card" onClick={() => openProject(p.id)}>
              ...
              <div className="proj-meta">
                {PLATFORM_LABEL[p.platform]} · {CATEGORY_LABEL[p.category]} · {p.step}/7 단계
              </div>
